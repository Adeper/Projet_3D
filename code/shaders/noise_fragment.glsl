...
float noise_1(vec2 p, float freq ){
	...
	xy = .5*(1.-cos(M_PI*xy));
	float a = rand_1((ij+vec2(0.,0.)));
	float b = rand_1((ij+vec2(1.,0.)));
	float c = rand_1((ij+vec2(0.,1.)));
	float d = rand_1((ij+vec2(1.,1.)));
	float x1 = mix(a, b, xy.x);
	float x2 = mix(c, d, xy.x);
	return mix(x1, x2, xy.y);
}

float pNoise_1(vec2 p, int res){
	float persistance = .5;
	float n = 0.;
	float normK = 0.;
	float f = 4.;
...
/* ----------- Classic Perlin 3D Noise Variante  ----------- 
 * by Stefan Gustavson (https://github.com/stegu/webgl-noise)
*/
vec4 fadeVar(vec4 t) {return t*t*t*(t*(t*6.0-15.0)+10.0);}

float cnoiseVar(vec4 P){
...
/* ----------- Classic Perlin periodic version  ----------- */
float cnoiseVarPer(vec4 P, vec4 rep){
...
float generateNoise(vec2 uv){
    ...
    for (int i = 0; i < noiseOctaves; i++){
        ...
        if (noiseType == 0)
            ...
            value = cnoiseVarPer(vec4(uv * frequency, 0.0, 0.0), vec4(1.0)); // Perlin 4D périodique
        else
            value = 0.0;
...
/* ----------- MAIN ----------- */
void main() {
    vec2 uv = gl_FragCoord.xy / screenWidth;
